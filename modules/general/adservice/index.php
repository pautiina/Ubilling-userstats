...
if ($us_config['ADSERVICE_ENABLED']) {
    ...
    function createTask($date, $login, $action, $param, $note) {
        ...
        $actionF = loginDB_real_escape_string($action);
        $paramF = loginDB_real_escape_string($param);
        $noteF = loginDB_real_escape_string($note);
    ...
    if (isset($_GET['delete_shedule'])) {
        if (!empty($_GET['delete_shedule'])) {
            ...
            $tag = strip_tags($tag);
            $tag = loginDB_real_escape_string($tag);
            $tag = vf($tag, 3);
